...
private:
    virtual QPolygonF points( double phase ) const = 0;//子类继承这个点传入时间变量重设点
...
public:
    Curve1()
    {
        ...
        QTransform transform;//是改变
        transform.scale( 1.5, 1.0 );//大小
        transform.translate( 1.5, 3.0 );//翻译上升
...
private:
    mutable QPolygonF  d_points;//允许在常成员函数内部修改
};
...
void Plot::updateCurves()
{
    ...
    qDebug()<<d_time.elapsed();
    for ( int i = 0; i < CurveCount; i++ )
        d_curves[3]->updateSamples( phase );
}
